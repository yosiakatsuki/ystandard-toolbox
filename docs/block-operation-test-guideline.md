...
テスト対象のブロックを挿入したら、**各設定パネルのテストを始める前に**以下を満たした状態にする。空のままテストすると実利用状況と乖離し、見た目・挙動の確認が不十分になるため。

### テキスト入力領域（RichText）

ブロックが備えるすべてのテキスト入力領域（RichText / placeholder があるテキスト欄）に、何らかのテキストを入力する。
...
- 許容するブロックが限定されている場合: その中から適切なブロックを追加
- 何でも許可されている場合: 段落ブロックを追加（ダミーテキストを入れる）
- 親子ブロック構成の場合: 親ブロックに子ブロックを 1 つ以上追加した状態でテスト開始

### この準備を行うタイミング

1. ブロックを挿入
2. **↑↑ ここで、テキスト入力とインナーブロック追加を実施 ↑↑**
3. 一度保存して「初期状態」を確定
4. この初期状態を起点に、各設定パネルのテストを順次実施

各テスト終了後、次のテストに入る前にこの初期状態（ブロック 1 つ + テキスト + インナーブロック）に戻す。完全空ではない点に注意。
...
11. 結果を operation.md に追記（必須、後述）
```

### 結果の書き込みタイミング（必須）

**1 つのテストが終わったら、その都度 operation.md に結果を追記する**。バッチ化・セッション終了時の一括書き込みはしない。

理由:
- セッション途中で中断が入っても、そこまでの結果が残る
- ユーザーがリアルタイムで進捗・結果を確認できる
- 発見した問題の原因追跡がしやすい（どの時点で起きたか明確）

テーブルの 1 行分を追記するだけの小さな Write 操作で OK。
...
### 詳細
| パネル | 設定項目 | パターン | エディタ | フロント | 再読込 | コンソール | 結果 | 備考 |
|---|---|---|---|---|---|---|---|---|
| リンク | リンクURL | サイト内 | ✅ | ✅ | ✅ | ✅ | OK | |
| リンク | 新しいタブで開く | ON → rel 自動設定 | ✅ | ✅ | ✅ | ✅ | OK | |
| バナー設定 | 背景色 | プリセット | ✅ | ✅ | ❌ | ✅ | NG | 再読込で検証エラー |
...

**列の意味**: `パネル` / `設定項目` / `パターン` は spec.md の見出し階層（h3 / h4 / h5）に対応。spec.md の該当箇所を特定しやすくするため。パネルをまたがない前段階のテスト（初期状態セットアップ・RichText 検証など）は「パネル」に `初期状態`・`RichText` などを入れ、「設定項目」は `-` にする。
