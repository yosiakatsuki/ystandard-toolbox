...
### L2: Playwright UI 自動テスト（自動・スポット）

- **手段**: Playwright MCP（`@playwright/mcp`）
- **実行頻度**: スポット実行（手動起動）
- **検証対象**:
...
- **得意**:
  ...
  - ブラウザフォーカスや外部拡張由来の click 発火不良に依存しにくい
- **不得意**:
  - 時間がかかる（1 件 7〜15 ツールコール）
  - 見た目の最終判断は L3 に委ねる
  - Media ライブラリやドラッグ操作は事前データ・座標調整が必要になる場合がある
...
| 検証内容 | L1 fixture | L2 Playwright UI | L3 手動 |
|---|---|---|---|
| 属性 → HTML 出力の正当性 | ✅ 主担当 | - | - |
...
L1 fixture の格納先は `test/integration/fixtures/blocks/{ブロックディレクトリ}/` 配下（ブロックごとのサブディレクトリに分割して格納する。詳細は [testing.md](testing.md#fixture-ディレクトリ構成) 参照）。
...
- L2: Playwright UI 自動テスト（排他・ラベル対応・条件表示・複雑 UI）
...
**重要**: spec.md はエージェントが自動更新しない。spec.md は「テスト仕様（繰り返し実行される検証項目）」として使い、セッションごとの進捗や結果は operation.md 側に記録する。仕様変更があった場合のみ、ユーザー承認のもとで spec.md を更新する。
...
banner-link/ystdtb__banner-link__link__new-tab-on.html
banner-link/ystdtb__banner-link__bg-color__preset-blue.html
banner-link/ystdtb__banner-link__bg-color__custom-hex.html
banner-link/ystdtb__banner-link__ratio__16-9.html
banner-link/ystdtb__banner-link__padding__shorthand-all-32.html
banner-link/ystdtb__banner-link__padding__shorthand-4corners.html
banner-link/ystdtb__banner-link__font-size__px-24.html
banner-link/ystdtb__banner-link__font-size__em-1-5.html
banner-link/ystdtb__banner-link__font-size__responsive-desktop-mobile.html
banner-link/ystdtb__banner-link__combo__photo-banner.html
banner-link/ystdtb__banner-link__combo__simple-card.html
```
...
パターン生成は L2 Playwright UI テストと同じルールを流用する（責任分担表の L1 列に ✅ / △ がある項目）。
...
3. `test/integration/fixtures/blocks/{ブロックディレクトリ}/` に .html ファイルとして保存（ディレクトリがなければ作成）
...
- L2 の主目的は **実 UI 経由の onChange / 排他 / 条件付き表示 / ラベル対応の確認**である。
- Playwright MCP では、クリック・入力・選択・キーボード操作など、ユーザー操作に相当する方法を優先する。
- `wp.data.dispatch('core/block-editor').updateBlockAttributes()` は、前提条件セットアップや状態確認を速くする目的に限定する。L2 テスト対象そのものの代替には使わない。
- 直接属性更新を使った場合は、operation.md に「UI 経由ではない」ことを明記し、UI onChange でしか発火しない副作用は未確認扱いにする。
- 保存後は、再読込して attribute / DOM / コンソールエラー / ブロック検証エラーを確認する。
- スクリーンショットは L3 の補助には使えるが、フロント見た目の最終判断は L3 手動確認で行う。
...
| Playwright MCP で UI 操作が安定しない | 手動確認または別操作手順に回す旨を記録（実装問題とは切り分ける） |
...
- 実機ブラウザで各ビューポートに切り替え
- 結果は `operation.md` にセッション単位で追記（Git 管理外）
...
Playwright MCP の evaluate 系操作で実行できる、L2 テストの品質向上・速度アップに役立つ snippet。
