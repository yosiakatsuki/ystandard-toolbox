...
yStandard テーマ使用時はテーマ側のカスタムプロパティを参照し、テーマなしの場合はフォールバック値を使用する。

| プロパティ | フォールバック値 | テーマ連携 | 用途 |
|---|---|---|---|
| `--ystdtb--color--gray-background` | `#f3f4f6` | `--ystd--site--background--light-gray` | 背景用の淡いグレー |
| `--ystdtb--color--gray-border` | `#e5e7eb` | `--ystd--site--border-color--gray` | ボーダー・区切り線 |
| `--ystdtb--color--gray-sub` | `#767676` | `--ystd--text-color--light-gray` | 日付・キャプション等の薄い補助テキスト |
| `--ystdtb--color--gray-text` | `#656565` | `--ystd--text-color--gray` | 補助テキスト |

テキスト色のコントラスト比（フォールバック値基準）:

| トークン | vs 白 | vs gray-background |
|---|---|---|
| `gray-text` (`#656565`) | 5.8:1 | 5.3:1 |
| `gray-sub` (`#767676`) | 4.5:1 | 4.2:1 |
...
color: var(--ystdtb--color--gray-sub);

/* 背景色 */
background-color: var(--ystdtb--color--gray-background);
