...
### ペーストで上書き保存（option 単位）

「設定値の確認」セクション内の各 option 表示直下にペースト用のテキストエリアがある。

操作フロー:

1. ソースサイトで「クリップボードにコピー」ボタンを押す（option の現在値が JSON テキストとしてコピーされる）
2. ターゲットサイト（別サイト・別タブ）で同じ option のテキストエリアにペースト
3. 「○○を上書き保存」ボタンを押すと即座に option へ書き戻される

エクスポート JSON とは異なり、**ペーストで受け付けるのは option の値そのもの**（コピーボタンで取得できる JSON）。

不正な入力時の挙動:

- 空文字 → エラー
- JSON パース失敗 → エラー
- パース成功だが配列以外（数値・文字列・null・bool）→ エラー（誤削除防止のため受け付けない）

確認ダイアログは出さない（コピー → ペーストの素早い往復を阻害しないため）。設定をクリアしたい場合は別途「削除」ボタンを使う。
