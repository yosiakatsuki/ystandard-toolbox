...
- Toolbox 本体が無効でも有効化はできるが、機能は実質的に Toolbox の option を読み書きするので Toolbox 有効環境で使うことを想定
...
- 親メニュー slug: `ystdtb-settings-v2`（`Config::ADMIN_MENU_SLUG_V2`）
- フック: `admin_menu`
- 関数: `add_submenu_page`
  - 親 slug: `ystdtb-settings-v2`
  - メニュータイトル: `Debug: 見出し設定`
  - 権限: `manage_options`
  - サブメニュー slug: `ystdtb-debug-heading-option`
  - `$position`: `999`（一番下に表示）
- **`ystdtb_plugin_settings_submenus` フィルターは使わない**（このフィルターは React SPA 用にコールバックが固定されているため、独自 PHP ページの描画には不適）
