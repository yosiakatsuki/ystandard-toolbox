...
function copyJson() {
  return src("./src/js/**/*.json").pipe(dest("js/"));
}

function cleanFiles(cb) {
  return del(["./ystandard-toolbox", "./build"], cb);
}

function cleanTemp(cb) {
  return del(["./ystandard-toolbox"], cb);
}

function deleteVendor(cb) {
  return del(["./ystandard-toolbox/vendor"], cb);
}
...
function copyUpdateJson() {
  return src("ystandard-toolbox.json").pipe(dest("build"));
}
...
exports.build = series(sass, sassBlocks, buildJs, copyJson);
