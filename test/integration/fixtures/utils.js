 ...
 * 例: test/integration/fixtures/blocks/box/ystdtb__box__type__label-out.html
 *
 * basename の prefix（`ystdtb__{block}__`）からサブディレクトリ名を自動算出する。
 */
/**
 ...
 *
 * `ystdtb__{block}__...` の `{block}` 部分をそのままディレクトリ名として返す。
 * ブロック名にハイフンを含むケース（例: `banner-link` / `sns-share` / `slider-item`）にも対応。
 *
...
function getBlockDirFromBasename( basename ) {
	const match = basename.match( /^ystdtb__([a-z0-9-]+)__/ );
