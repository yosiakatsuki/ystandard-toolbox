...
- **タイプ切替時のリセット**:
  - `LabelType.handleOnChange` で `labelContents` を上書き：`'icon' === value` → `'bookmark'`、それ以外 → `''`
...
| ラベル一括設定 | 角丸 | `labelBorderRadius` |
| ラベル一括設定 | タイプ | `labelType` / `labelContents`（タイプ切替で上書き） |
...
- [ ] `ystdtb__timeline__label-type__none` — `labelType: ''`（デフォルト・明示なし、ラベル中身空）
- [ ] `ystdtb__timeline__label-type__icon` — `labelType: 'icon'` + `labelContents: 'bookmark'`
- [ ] `ystdtb__timeline__label-type__text` — `labelType: 'text'` + `labelContents: 'A'`

#### アイコン（`labelContents`、`labelType=icon` 前提）

代表 3 種で網羅。全 SVG パスは L2 / L3 で目視。

- [ ] `ystdtb__timeline__label-icon__bookmark` — `labelContents: 'bookmark'`（タイプ icon のデフォルト）
- [ ] `ystdtb__timeline__label-icon__star` — `labelContents: 'star'`
- [ ] `ystdtb__timeline__label-icon__heart` — `labelContents: 'heart'`

#### テキスト（`labelContents`、`labelType=text` 前提）

- [ ] `ystdtb__timeline__label-text__one-char` — `labelContents: '1'`（円形ラベル、`has-long-text` なし）
- [ ] `ystdtb__timeline__label-text__multi-char` — `labelContents: 'NEW'`（`has-long-text` 付与）

#### ラベルサイズ（`labelSize`）

- [ ] `ystdtb__timeline__label-size__40px` — `labelSize: '40px'`（icon ラベル）
- [ ] `ystdtb__timeline__label-size__60px` — `labelSize: '60px'`（icon ラベル）

#### 角丸（`labelBorderRadius`、default `50px`）

- [ ] `ystdtb__timeline__label-radius__zero` — `labelBorderRadius: '0'`（デフォルトを上書き、角なし）
- [ ] `ystdtb__timeline__label-radius__8px` — `labelBorderRadius: '8px'`

#### ラベル太さ（`labelBold`、`labelType=text` 前提）

- [ ] `ystdtb__timeline__label-bold__false` — `labelBold: false`（デフォルト）
- [ ] `ystdtb__timeline__label-bold__true` — `labelBold: true`（style に `font-weight:700` 出力）

#### 文字・アイコンサイズ（`labelFontSize` / `customLabelFontSize`）

- [ ] `ystdtb__timeline__label-font__preset` — `labelFontSize: 'large'`（icon ラベル、`has-large-font-size` クラス付与）
- [ ] `ystdtb__timeline__label-font__custom` — `customLabelFontSize: '28px'`（icon ラベル、style に `font-size:28px` 出力）

#### 文字・アイコン色（`labelColor` / `customLabelColor`）

- [ ] `ystdtb__timeline__label-color__preset` — `labelColor: 'ys-blue'`（icon ラベル、`has-text-color has-ys-blue-color` クラス）
- [ ] `ystdtb__timeline__label-color__custom` — `customLabelColor: '#0284c7'`（icon ラベル、`has-text-color` クラス + style）

#### ラベル背景色（`labelBackgroundColor` / `customLabelBackgroundColor`）

- [ ] `ystdtb__timeline__label-bg__preset` — `labelBackgroundColor: 'ys-light-blue'`
- [ ] `ystdtb__timeline__label-bg__custom` — `customLabelBackgroundColor: '#fef3c7'`

#### ラベル枠線の太さ（`labelBorderSize`）

- [ ] `ystdtb__timeline__label-border__1px` — `labelBorderSize: '1px'` + `labelBorderColor: 'ys-blue'`
- [ ] `ystdtb__timeline__label-border__2px` — `labelBorderSize: '2px'` + `labelBorderColor: 'ys-blue'`

#### ラベル枠線の色（`labelBorderColor` / `customLabelBorderColor`）

- [ ] `ystdtb__timeline__label-border-color__preset` — `labelBorderSize: '2px'` + `labelBorderColor: 'ys-purple'`
- [ ] `ystdtb__timeline__label-border-color__custom` — `labelBorderSize: '2px'` + `customLabelBorderColor: '#dc2626'`

### 子 `ystdtb/timeline-item` > タイムライン設定

#### コンテンツ間の余白（`contentsInnerMargin`、default `normal`）

- [ ] `ystdtb__timeline__contents-margin__small` — `contentsInnerMargin: 'small'`
- [ ] `ystdtb__timeline__contents-margin__large` — `contentsInnerMargin: 'large'`

#### 枠線の太さ（`contentsBorderSize`）

- [ ] `ystdtb__timeline__border-size__1px` — `contentsBorderSize: '1px'` + `contentsBorderColor: 'ys-gray'`
- [ ] `ystdtb__timeline__border-size__4px` — `contentsBorderSize: '4px'` + `contentsBorderColor: 'ys-gray'`

#### 線の色（`contentsBorderColor` / `customContentsBorderColor`）

- [ ] `ystdtb__timeline__border-color__preset` — `contentsBorderColor: 'ys-green'`
- [ ] `ystdtb__timeline__border-color__custom` — `customContentsBorderColor: '#7c3aed'`

#### コンテンツ上部余白（`contentMarginTop`）

- [ ] `ystdtb__timeline__margin-top__zero` — `contentMarginTop: '0px'`
- [ ] `ystdtb__timeline__margin-top__20px` — `contentMarginTop: '20px'`

### 子アイテム個別設定（複数子で異なる属性）

- [ ] `ystdtb__timeline__child__mixed-label` — 子 3 個に異なる `labelType`（なし / text NEW / icon star）+ ラベル色違い
- [ ] `ystdtb__timeline__child__mixed-border-color` — 子 3 個に異なる `contentsBorderColor`（ys-blue / ys-green / ys-red）
- [ ] `ystdtb__timeline__child__mixed-contents-margin` — 子 3 個に異なる `contentsInnerMargin`（small / normal / large）

### 代表的な組み合わせパターン（examples HTML の組み合わせ例を流用）

- [ ] `ystdtb__timeline__combo__blog-history` — シンプル（ラベルなし、3 アイテム）
- [ ] `ystdtb__timeline__combo__step-guide` — 数字テキスト（labelBold + 背景色 + 文字色プリセット、3 アイテム）
- [ ] `ystdtb__timeline__combo__award` — アイコン（award）+ labelSize 50px + 角丸 8px + 背景色プリセット
- [ ] `ystdtb__timeline__combo__service-flow` — テキスト「STEP 1」+ has-long-text + カスタム色 + カスタム背景 + customLabelFontSize
- [ ] `ystdtb__timeline__combo__event` — 子 3 個に異なるアイコン（bookmark/star/heart）+ 異なる色（labelColor / contentsBorderColor）
...
- 本仕様の fixture 数はおよそ 35 件（label-type 3 / label-icon 3 / label-text 2 / label-size 2 / label-radius 2 / label-bold 2 / label-font 2 / label-color 2 / label-bg 2 / label-border 2 / label-border-color 2 / contents-margin 2 / border-size 2 / border-color 2 / margin-top 2 / child 3 / combo 5）
