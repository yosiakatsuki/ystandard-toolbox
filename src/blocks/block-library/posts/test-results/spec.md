...
| アスペクト比（モバイル） | `thumbnailRatioMobile` | string | undefined（未選択時はPC設定を継承） |
...
実装は `phpunit/blocks/test-posts.php`。`Posts_Block::get_instance()->render_callback($attributes)` の戻り値 HTML を `DOMDocument` + `DOMXPath` で解析し、要素・クラス・inline style・件数・順序を検証する。
