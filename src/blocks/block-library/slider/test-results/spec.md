...
`save()` を持つ static block かつ、属性 → `data-slider-options` JSON 文字列の組み立てロジック（`getSliderOptions()`）が複雑なため、四層構成（L0 Jest unit / L1 fixture / L2 Chrome UI / L3 手動）で動作を検証する。
...
L1 fixture / L0 Jest unit でカバーできない UI 経由の挙動のみを Chrome 拡張で検証する。
...
- [ ] 「設定の組み合わせ例」5 パターン（基本 / ヒーロー / 商品カード / フェード / 16:9 アスペクト比）が想定通り動作
- [ ] 「子ブロックの種類」3 パターン（cover / video / slider-item+box）が想定通り表示
- [ ] 「基本設定」: エフェクト 4 種（slide / fade / coverflow / cube）の動作、スピード変化、ループ OFF、イージング関数
- [ ] 「自動再生」: 自動再生 OFF / 表示時間 15 秒 / マウスホバー停止 / 操作後再開 OFF / 逆方向
- [ ] 「サイズ」: アスペクト比 7 種、高さ単一値、高さレスポンシブ（3 デバイス）
- [ ] 「スライド表示数」: 全デバイス共通 5 種、デバイス別 2 種
- [ ] 「ナビゲーション」: プリセット色（ys-red）、カスタム HEX 色
- [ ] 「ページネーション」: ダイナミック / プログレスバー / fraction、カスタム色
- [ ] 「配置」: 幅広（alignwide）

### Swiper 機能の実動作

- [ ] 自動再生がフロントで動く（エディター内では Swiper は走らない）
- [ ] ナビゲーション矢印クリックで前後スライド
- [ ] ページネーションクリックでスライド遷移（bullets）
- [ ] ループ ON 時に最終スライドから最初に戻る
- [ ] エフェクト fade で重なるアニメーション
- [ ] エフェクト coverflow で 3D 風アニメーション
- [ ] エフェクト cube でキューブ回転アニメーション
...
- [ ] スライダー内に画像ブロック（`core/image`）を入れた場合の表示
- [ ] スライダー内にカバーブロック（`core/cover`）を入れた場合の表示
- [ ] スライダー内に動画ブロック（`core/video`）を入れた場合の表示
- [ ] スライダー内にスライダーアイテム（`ystdtb/slider-item`）を入れて、その中に他のブロック（BOX 等）を入れた場合の表示

### コンソールエラー

- [ ] 操作中・属性更新中ともに `console.error` / 例外発生なし
- [ ] フロントで Swiper の初期化エラーなし
