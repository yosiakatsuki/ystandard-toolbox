...
読み込みは `enqueue_block_assets` で行う。このフックは**フロントとエディターキャンバス(iframe)の両方**で実行される
（WordPress 6.3 以降、このフックで追加したアセットは iframe 化されたエディターにも読み込まれる）。

### エディターでの見え方

エディターキャンバスは iframe なので、フロントと同じメディアクエリ CSS を読み込むだけで
**デバイスプレビューの幅に追従して改行が切り替わる**。JS でデバイス種別を見る必要はない。

| 状態 | 改行される種類 |
| --- | --- |
| 通常編集（キャンバス幅 ≥ 1024px） | PCのみ改行 |
| タブレットプレビュー（781px） | タブレットのみ改行 |
| モバイルプレビュー（479px） | モバイルのみ改行 |

デバイスプレビューの幅は `block-editor/src/components/use-resize-canvas/index.js` で
タブレット 781px / モバイル 479px と定義されており、Toolbox のブレークポイント区分と一致する。

**注意**: デスクトッププレビューはキャンバスの実幅（ウィンドウ幅 − サイドバー）になる。
ディスプレイが小さくキャンバスが 1024px 未満になると、デスクトッププレビューでもタブレット扱いになる。
これはテーマのレスポンシブ CSS がエディターで見せる挙動と同じ。

改行が非表示のときも位置と種類が分かるよう、`::before` の生成内容で2行のバッジを表示する。
上段は`改行`、下段は対象デバイス（`スマホ` / `タブレット` / `PC`）とする。
バッジは改行の表示状態にかかわらず常に表示される。

ラベルはCSSへ直接記述せず、`index.php`の翻訳関数で取得してCSSカスタムプロパティへ渡す。
エディター用CSSは、翻訳済みの2つの文字列を改行コード`\A`で連結して表示する。
これにより保存HTMLを変更せず、翻訳ファイルによる国際化へ対応できる。
...
- 段落の途中で「モバイルのみ改行」→ 上段`改行`・下段`スマホ`のバッジが出る（通常編集の幅では改行されない）
- エディター上部のプレビュー切り替えでモバイル・タブレットにすると、対象の改行だけが改行される
