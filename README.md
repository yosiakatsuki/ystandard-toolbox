...
yStandard Toolbox のメジャーバージョンアップ。約2年に渡る内部刷新と機能改修を反映した大規模アップデート。
...
##### yStandard以外のテーマでの対応強化

- [追加] フォント設定をyStandard以外のテーマでも利用可能に
- [追加] 投稿一覧ブロックをyStandard以外のテーマでも利用可能に
- [調整] yStandardの連携が必要な機能はyStandard以外の環境で設定を非表示化

##### 見出しデザイン機能の刷新

- [追加] 見出しデザイン機能を全面刷新
- [追加] 見出しレベル別のスタイル割り当て機能追加
- [追加] ブロックスタイルとの紐づけに対応（見出しブロック、段落ブロックのスタイルとして選択できる機能追加）
- [追加] `::before` / `::after` 疑似要素の編集機能（アイコン選択・文字・背景等）
- [追加] 文字・背景画像・枠線・余白・影・サイズ系の設定項目を全面拡充
- [追加] v1 → v2 見出しデザイン設定の移行ツールの追加

##### ブロック関連

- [追加] スライダーブロック：自動再生の逆方向再生設定
- [追加] 記事一覧ブロック：「概要文の最大行数」設定（デフォルト:2）
- [変更] 記事一覧ブロック：yStandard 連携を撤廃
- [変更] ボックスブロック：レスポンシブ余白設定の挙動を統一（破壊的変更セクション参照）

##### CTA（行動喚起）機能

- [追加] CTA 設定の表示 / 非表示切り替えボタン
- [変更] CTA の順序変更をドラッグ&ドロップに対応
- [変更] CTA 設定画面の UI を刷新

##### その他

- [修正] 全幅ブロックで画面サイズ別非表示機能を設定するとエディターで横スクロールが表示される点の修正
- [修正] FAQ ブロックでフォントサイズの指定を変えるとブロックエラーが出る点の修正
...
- [変更] 定義リストブロック（dt）：フォントサイズ・外側余白・内側余白の設定に破壊的変更
- [変更] 定義リストブロック（dd / シンプル）：フォントサイズ・外側余白・内側余白の設定に破壊的変更
- [変更] 定義リストブロック（dl-column / 横並び）：縦並び動作・dt のサイズ・枠線・外側余白の設定に破壊的変更
...
- [変更] 記事一覧ブロック：yStandard 連携を削除し Toolbox 独自のロジック・スタイルに変更
- [削除] 記事一覧ブロック：「概要文の長さ」設定（→「概要文の最大行数」設定に置き換え）
- [変更] ボックスブロック：レスポンシブ余白のメディアクエリ処理を PHP 側（`Styles::add_media_query_*`）に統一。従来 768〜1023px の画面幅ではタブレット用の余白設定が反映されていなかったが、ブレークポイント定義（mobile / tablet / desktop）に揃うように修正

#### 移行時の注意

- 見出しデザインは v1 → v2 で構造が大きく変わっているため、専用の移行ツールを用意しています。設定画面から実行してください。すぐに移行できない場合は「下位互換モード」で v1 と同じ動作のまま継続利用できます
- 定義リストブロック（dl-column）で旧クラス `.is-not-stacked-on-*` / `dl-column-width-*` を使用している投稿は、エディターで開いて再保存することを推奨。
- `ystdtb_css_breakpoints` フィルターをカスタマイズしている場合、新しいキー名・単位付きの値に書き換えが必要
- 記事一覧ブロックの「概要文の長さ」設定は削除されています。デザイン要件に応じて新しい「概要文の最大行数」設定への置き換えが必要です
