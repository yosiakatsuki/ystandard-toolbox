...
- [変更] 定義リストブロック（dl-column / 横並び）：縦並び動作・dt のサイズ・枠線・外側余白の設定に破壊的変更
  - `.is-not-stacked-on-tablet` / `.is-not-stacked-on-mobile` の動作変更。対象スタイルが使用されている投稿の編集画面を開く画面を用意.
  - `dl-column-width-desktop` / `dl-column-width-tablet` / `dl-column-width-mobile` スタイルの動作変更。対象スタイルが使用されている投稿の編集画面を開く画面を用意.
